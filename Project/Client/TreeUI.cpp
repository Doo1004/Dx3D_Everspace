...
void TreeNode::render_update()
{
    ...
    if (ImGui::TreeNodeEx(strFinalName.c_str(), flag))
    {
        ...
        if (ImGui::BeginPopup("EditNode"))
        {
            ImGui::Text("EditNode");
            ImGui::Separator();
            if (ImGui::Selectable("DeleteObject"))
            {
                if (m_Owner->GetSelectedNode())
                {
                    ...
                    CGameObject* pDeleteObj = (CGameObject*)m_Owner->GetSelectedNode()->GetData();
                    DestroyObject(pDeleteObj);
                }
            }

            if (ImGui::Selectable("CreatePrefab"))
            {
                if (m_Owner->GetSelectedNode())
                {
                    CGameObject* pSelectedObj = (CGameObject*)m_Owner->GetSelectedNode()->GetData();
                    CreatePrefab(pSelectedObj);
                }
            }
...
void TreeNode::CreatePrefab(CGameObject* _Object)
{
    Ptr<CPrefab> pPrefab = new CPrefab(true);
    CGameObject* cObject = _Object->Clone();
    pPrefab->RegisterProtoObject(cObject);
    pPrefab->SetName(cObject->GetName());

    wstring Name = _Object->GetName();

    CResMgr* pRes = CResMgr::GetInst();
    pRes->AddRes(Name, pPrefab);
