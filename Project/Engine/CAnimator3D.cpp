...
void CAnimator3D::SaveToLevelFile(FILE* _pFile)
{
	...
	int clipUpdateTimeSize = m_vecClipUpdateTime.size();
	fwrite(&clipUpdateTimeSize, sizeof(int), 1, _pFile);
	for (int i = 0; i < clipUpdateTimeSize; i++) {
		fwrite(&m_vecClipUpdateTime[i], sizeof(float), 1, _pFile);
	}
...
void CAnimator3D::LoadFromLevelFile(FILE* _pFile)
{
	...
	int clipUpdateTimeSize;
	fread(&clipUpdateTimeSize, sizeof(int), 1, _pFile);
	m_vecClipUpdateTime.resize(clipUpdateTimeSize);
	for (int i = 0; i < clipUpdateTimeSize; i++) {
		fread(&m_vecClipUpdateTime[i], sizeof(float), 1, _pFile);
	}
