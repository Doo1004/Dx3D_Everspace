...
void CCamera::finaltick()
{
	...
	m_Frustum.SetCameraDebugShape(m_bShowCamereDebug);
	m_Frustum.SetCameraDebugScale(m_CameraDebugScale);
	m_Frustum.finaltick();
		...
		if (m_iLayerMask & (1 << i))
		{
			...
			for (size_t j = 0; j < vecObject.size(); ++j)
			{
				...
				if (pRenderCom->IsUseFrustumCheck())
				{
					...
					if (false == m_Frustum.FrustumCheckBySphere(vWorldPos, pRenderCom->GetBounding(), pRenderCom->IsUseShowDebugShape()))
						continue;
				}
