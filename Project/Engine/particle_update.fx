            ...
            while (0 < SpawnCount)
            {
                ...
                {   
                    ...
                    if (ModuleData.AddVelocity)
                    {
                        ...
                        {
                            // pObject의 z축 방향을 기준으로 한 속도 벡터
                            float3 vVelocity = float3(0, 0, 1);
                            
                            // pObject의 회전 행렬을 이용하여 속도 벡터를 회전시킨다.
                            float4 vRot = mul(float4(vVelocity, 0.f), ObjectRot);

                            // 최종 속도를 구한다.
                            particle.vVelocity.xyz = vRot.xyz * ModuleData.Speed;
                        }
                    }          
