...
void DrawDebugCube(Vec3 _vWorldPos, Vec3 _vWorldScale, Vec4 _vColor, Vec3 _vRotation, float _fTime)
{
	tDebugShapeInfo info = {};
	...
	info.fMaxTime = _fTime;
	info.vWorldPos = _vWorldPos;
	info.vWorldScale = Vec3(_vWorldScale.x, _vWorldScale.y, _vWorldScale.z);
	info.vWorldRotation = _vRotation;
	info.vColor = _vColor;
