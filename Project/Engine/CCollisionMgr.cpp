...
void CCollisionMgr::CollisionBtwObject(CGameObject* _LeftObject, CGameObject* _RightObject)
{
	if ((_LeftObject->Collider2D() && _RightObject->Collider2D()))
	{
		...
		// 두 충돌체가 지금 충돌 중인지 확인
		if (CollisionBtwCollider2D(_LeftObject->Collider2D(), _RightObject->Collider2D()))
		{
	...
	if ((_LeftObject->Collider3D() && _RightObject->Collider3D()))
	{
		...
		// 두 충돌체가 지금 충돌 중인지 확인
		if (CollisionBtwCollider3D(_LeftObject->Collider3D(), _RightObject->Collider3D()))
		{
			// 이전에 충돌한 적이 있고, 둘중 하나 이상이 삭제 예정이라면
			if (bDead && iter->second)
			{
...
bool CCollisionMgr::CollisionBtwCollider3D(CCollider3D* _pLeft, CCollider3D* _pRight)
{
	COLLIDER3D_TYPE LeftType = _pLeft->Collider3D()->GetCollider3DType();
	COLLIDER3D_TYPE RightType = _pRight->Collider3D()->GetCollider3DType();

	if (LeftType == COLLIDER3D_TYPE::CUBE && RightType == COLLIDER3D_TYPE::CUBE)
		return CollisionBtwCube(_pLeft, _pRight);
	else if (LeftType == COLLIDER3D_TYPE::SPHERE && RightType == COLLIDER3D_TYPE::SPHERE)
		return CollisionBtwSphere(_pLeft, _pRight);
	else if (LeftType == COLLIDER3D_TYPE::CUBE && RightType == COLLIDER3D_TYPE::SPHERE)
		return CollisionBtwCubeToSphere(_pLeft, _pRight);
	else if (LeftType == COLLIDER3D_TYPE::SPHERE && RightType == COLLIDER3D_TYPE::CUBE)
		return CollisionBtwCubeToSphere(_pRight, _pLeft);

	return false;
}


bool CCollisionMgr::CollisionBtwCube(CCollider3D* _pLeft, CCollider3D* _pRight)
{
	Vec3 arrLocal[8] =
	{
		Vec3(-0.5f, 0.5f, 0.5f),
	...
	// 앞면
	arrProj[2] = XMVector3TransformCoord(arrLocal[2], _pLeft->GetColliderWorldMat()) - XMVector3TransformCoord(arrLocal[3], _pLeft->GetColliderWorldMat());
	arrProj[3] = XMVector3TransformCoord(arrLocal[4], _pLeft->GetColliderWorldMat()) - XMVector3TransformCoord(arrLocal[3], _pLeft->GetColliderWorldMat());

	// 왼쪽면
	arrProj[4] = XMVector3TransformCoord(arrLocal[3], _pLeft->GetColliderWorldMat()) - XMVector3TransformCoord(arrLocal[0], _pLeft->GetColliderWorldMat());
	arrProj[5] = XMVector3TransformCoord(arrLocal[7], _pLeft->GetColliderWorldMat()) - XMVector3TransformCoord(arrLocal[0], _pLeft->GetColliderWorldMat());


	// 우측 충돌체도 똑같이
	// 윗면
	arrProj[6] = XMVector3TransformCoord(arrLocal[1], _pRight->GetColliderWorldMat()) - XMVector3TransformCoord(arrLocal[0], _pRight->GetColliderWorldMat());
	arrProj[7] = XMVector3TransformCoord(arrLocal[3], _pRight->GetColliderWorldMat()) - XMVector3TransformCoord(arrLocal[0], _pRight->GetColliderWorldMat());

	// 앞면
	arrProj[8] = XMVector3TransformCoord(arrLocal[2], _pRight->GetColliderWorldMat()) - XMVector3TransformCoord(arrLocal[3], _pRight->GetColliderWorldMat());
	arrProj[9] = XMVector3TransformCoord(arrLocal[4], _pRight->GetColliderWorldMat()) - XMVector3TransformCoord(arrLocal[3], _pRight->GetColliderWorldMat());

	// 왼쪽면
	arrProj[10] = XMVector3TransformCoord(arrLocal[3], _pRight->GetColliderWorldMat()) - XMVector3TransformCoord(arrLocal[0], _pRight->GetColliderWorldMat());
	arrProj[11] = XMVector3TransformCoord(arrLocal[7], _pRight->GetColliderWorldMat()) - XMVector3TransformCoord(arrLocal[0], _pRight->GetColliderWorldMat());
	...
	for (int i = 0; i < 12; ++i)
	{
		...
		// 12개의 표면백터를 지정된 투영축으로 투영시킨 거리의 총합 / 2 !! 수정 : 4
		float fProjDist = 0.f;
		for (int j = 0; j < 12; ++j)
		{
			fProjDist += fabsf(arrProj[j].Dot(vProj));
...
bool CCollisionMgr::CollisionBtwSphere(CCollider3D* _pLeft, CCollider3D* _pRight)
{
	// 구의 x 크기를 기준으로 반지름을 잡음(완전한 구여야 함)
	float fLeftRadius = _pLeft->GetOffsetScale().x / 2;
	float fRightRadius = _pRight->GetOffsetScale().x / 2;

	// 두 충돌체의 중심점을 구함
	Vec3 vCenter = XMVector3TransformCoord(Vec3(0.f, 0.f, 0.f), _pRight->GetColliderWorldMat()) - XMVector3TransformCoord(Vec3(0.f, 0.f, 0.f), _pLeft->GetColliderWorldMat());
	float distance = fabsf(vCenter.Length());

	if (distance > (fLeftRadius + fRightRadius))
		return false;

	return true;
}

bool CCollisionMgr::CollisionBtwCubeToSphere(CCollider3D* _pCube, CCollider3D* _pSphere)
{
	Vec3 arrLocal[8] =
	{
	...
	Vec3 vCubeCenter = XMVector3TransformCoord(Vec3(0.f, 0.f, 0.f), _pCube->GetColliderWorldMat());

	float fSphereRadius = _pSphere->GetOffsetScale().x / 2;
	Vec3 vSphereCenter = XMVector3TransformCoord(Vec3(0.f, 0.f, 0.f), _pSphere->GetColliderWorldMat());
...
bool CCollisionMgr::IsLayerColliding(UINT _left, UINT _right)
{
	UINT iRow = (UINT)_left;
	UINT iCol = (UINT)_right;
	...
	return (m_matrix[iRow] & (1 << iCol)) != 0;
}


void CCollisionMgr::ModifyLayerCollision(UINT _left, UINT _right, bool _collide)
{
	UINT iRow = (UINT)_left;
	UINT iCol = (UINT)_right;
	...
	if (_collide)
	{
		m_matrix[iRow] |= (1 << iCol); // Set the bit to enable collision
	}
	else
	{
		m_matrix[iRow] &= ~(1 << iCol); // Clear the bit to disable collision
	}
}
