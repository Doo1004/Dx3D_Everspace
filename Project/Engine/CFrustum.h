#pragma once
#include "CEntity.h"

class CCamera;

enum FACE_TYPE
{
    FT_NEAR,
    FT_FAR,
    FT_LEFT,
    FT_RIGHT,
    FT_TOP,
    FT_BOT,
    FT_END,
};

class CFrustum :
    public CEntity
{
private:
    CCamera* m_pOwner;
    Vec4        m_arrFace[FT_END];
    Vec3        m_arrProj[8];
    bool        m_bShowCameraDebugShape;
    float       m_CameraDebugScale;

public:
    void finaltick();

public:
    bool FrustumCheckByPoint(Vec3 _vWorldPos);
    bool FrustumCheckBySphere(Vec3 _vWorldPos, float _fRadius, bool _bDebugUse);

    void SetCameraDebugShape(bool _bShow) { m_bShowCameraDebugShape = _bShow; }
    void SetCameraDebugScale(float _fScale) { m_CameraDebugScale = _fScale; }

public:
    CLONE(CFrustum);
    CFrustum(CCamera* _pOwner);
    ~CFrustum();
};

