...
void CParticleSystem::finaltick()
{
	...
	Vec3 vDebugShapePos = Transform()->GetWorldPos();
	Vec2 vRectScale = Vec2(m_ModuleData.vBoxShapeScale.x, m_ModuleData.vBoxShapeScale.y);
	Vec4 vDebugShapeColor = Vec4(0.f, 1.f, 0.f, 1.f);
	Vec3 vDebugShapeRot = Transform()->GetRelativeRot();

	if (m_bShowDebugShape)								// DebugShape 보일지 말지
	{
		if (m_ModuleData.SpawnShapeType == 0)			// Rect
			DrawDebugRect(vDebugShapePos, vRectScale, vDebugShapeColor, vDebugShapeRot, 0.f);
		//else if (m_ModuleData.SpawnShapeType == 1)	// Circle은 그냥 쓰지말자
		//	DrawDebugCircle(vDebugShapePos, );
		else if (m_ModuleData.SpawnShapeType == 2)		// Cube
			DrawDebugCube(vDebugShapePos, m_ModuleData.vBoxShapeScale, vDebugShapeColor, vDebugShapeRot, 0.f);
	}
	...
	m_UpdateCS->SetParticleObjectRot(Transform()->GetWorldRotation());
