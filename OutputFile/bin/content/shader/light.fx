...
PS_OUT PS_PointLightShader(VS_OUT _in)
{
    ...
    if (length(vLocalPos) <= 0.5f)
    {
        ...
        float distanceFactor = 1.0f - length(vLocalPos) / 0.5f;
        
        output.vDiffuse = lightcolor.vDiffuse * distanceFactor + lightcolor.vAmbient * distanceFactor;
        output.vSpecular = lightcolor.vSpecular * distanceFactor;
