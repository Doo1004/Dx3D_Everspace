...
PS_OUT PS_Decal(VS_DECAL_OUT _in)
{
    ...
    float4 g_SetDecalColor = g_vec4_0;
    float4 vSetDecalColorData = float4(g_SetDecalColor.xyz, -g_SetDecalColor.a);
        ...
        && abs(vLocalPos.z) <= 0.5f)
    {
        if (g_btex_0)
        {
            ...
            float4 vSampleColor = g_tex_0.Sample(g_sam_0, vSampleUV) + vSetDecalColorData;
